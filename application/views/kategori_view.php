...
<head>
    ...
    <title>Kategori: <?= htmlspecialchars($kategori_info['nama']) ?> - CMS Portal</title>
    ...
    <style>
        ...
        .artikel-card { border: none; border-radius: 15px; overflow: hidden; transition: transform 0.3s, box-shadow 0.3s; height: 100%; box-shadow: 0 5px 15px rgba(0,0,0,0.08); }
        .artikel-card:hover { transform: translateY(-10px); box-shadow: 0 15px 35px rgba(0,0,0,0.15); }
...
<body>
    ...
    <div class="page-header">
        <div class="container">
            <h1><i class="fas fa-folder-open"></i> Kategori: <?= htmlspecialchars($kategori_info['nama']) ?></h1>
            <?php if($kategori_info['deskripsi']): ?>
                <p class="lead"><?= htmlspecialchars($kategori_info['deskripsi']) ?></p>
            <?php endif; ?>
        </div>
    </div>
    ...
    <div class="container mb-5">
        <?php if(empty($artikel)): ?>
            <div class="alert alert-info">
                <i class="fas fa-info-circle"></i> Tidak ada artikel dalam kategori ini.
            </div>
                ...
                <?php foreach($artikel as $item): ?>
                    <div class="col-md-4 mb-4">
                        <div class="card artikel-card">
                            <div class="card-body">
                                <h5><a href="<?= base_url('welcome/artikel/'.$item['slug']) ?>" class="text-decoration-none"><?= htmlspecialchars($item['judul']) ?></a></h5>
                                <p class="text-muted"><?= word_limiter(strip_tags($item['konten']), 20) ?></p>
                                <small class="text-muted">
                                    <i class="far fa-calendar"></i> <?= date('d M Y', strtotime($item['tanggal_dibuat'])) ?> | 
                                    <i class="far fa-eye"></i> <?= $item['views'] ?> views
                                </small>
                                <div class="mt-3">
                                    <a href="<?= base_url('welcome/artikel/'.$item['slug']) ?>" class="btn btn-read-more btn-sm">Baca <i class="fas fa-arrow-right"></i></a>
                                </div>
                            </div>
                        </div>
                    </div>
